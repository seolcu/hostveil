...
hostveil is a single-binary Linux security scanner (Docker Compose +
Trivy + Lynis) with an embedded Bubble Tea TUI and a no-build Web UI.
The codebase is small (~13.5k LoC Go) but the fix engine, the scoring
model, and the Web UI's XSS surface have design rules that are easy to
break. Read this file before you touch anything.
...
1. **Read the relevant doc** — pick from the index below before reading
   code. Do not skim the codebase blind.
...
5. **Verify CI** — after push, watch the `build`, `test-installer`,
   and `e2e` jobs. Do not declare done until all three are green.
6. **Clean up** — see the "Cleanup" checklist.

If you are blocked or unsure, say so. Do not guess at the project's
intent. The owner prefers a short "I need X, here is why" over a
...
| [`SECURITY.md`](SECURITY.md) | Before touching the Web UI, the fix engine, the port reclaim, or anything that touches the host filesystem. Threat model, what hostveil protects against, how to report. |
...
Targeted runs (faster feedback while iterating):
...
The installer matrix is run by CI only; do not run it locally unless
asked. The `hostveil-e2e` binary and `test/e2e/test-results/` are
build artefacts — gitignored, do not commit them.
...
```
cmd/hostveil/        — main, subcommands, signal handling
internal/
├── domain/          — types, scoring, scan progress
├── scan/            — single-tool dispatcher
├── trivy/           — Trivy adapter (config + image)
...
adding code, follow them. If you are editing code that violates them,
fix the violation as part of your change.
...
- `sync.RWMutex` for any field shared between the TUI/Web UI and the
  scanner goroutines. The canonical example is `domain.ScanProgress`.
...
- **Every value rendered into `innerHTML` MUST be HTML-escaped.**
  The shared helper is `escapeHTML(...)`. Browser-decoded `data-*`
  attribute values must be re-escaped on read — the browser has
  already done entity-decoding once. (`app.js:section()` and the
  "View more" / "View less" toggle are the historical hot spots;
  the XSS regression test in `test/e2e/specs/xss.spec.ts` covers
  them.)
...
user picks between. The user picks ONE of N options, or any subset.
Each action must address the concern **independently** — none of
them depend on the others being applied.

**Counter-example (the KRNL-6000 mistake).** Originally registered
as 1 bundled action that applied 6 sysctls together. Wrong: it
forced the user to accept all 6 or none. A user running a router
may want `syncookies=1` but NOT `accept_source_route=0`. Reverted
...
- Lynis findings use stable test IDs (`AUTH-9286`) in the finding
  ID: `"lynis.AUTH-9286"`. The `lynis.` prefix is the registration
  key for `fix.Registry`.
...
- Adding a new public symbol to the Web UI or the TUI (changes the
  user-visible surface; the maintainer will want to review the UX
  decision).
...
- Commit secrets, API keys, tokens, or anything that resembles a
  ...
  `${{ secrets.GITHUB_TOKEN }}` — never hardcoded.
...
- Inject any value into `innerHTML` without `escapeHTML(...)`.
  Browser-decoded `data-*` attribute values must be re-escaped on
  read. (See "Security" below.)
...
- The fix engine (`internal/fix`) — any change that runs `Apply`
...
- **Out-of-range `action_index` in `/api/fix`.** The handler
  ...
  will panic the process. The handler now bounds-checks up
  front and returns a clear error. Regression test:
...
- **Commit message format:** `area: imperative description`.
  `area` is one of `fix`, `feat`, `docs`, `test`, `refactor`,
  `chore`. 50 chars or less. Examples:
...
- **PR body:** describe *why* the change is needed. The diff shows
  *what* changed; do not restate it in prose.
- **Tests must fail before your fix and pass after.** No
  "harmless refactor" PRs that are not actually testable.
- **Run the four local checks before pushing.** If CI fails
  after push, fix and re-push immediately.
- **Watch CI until all jobs are green.** Use `gh run watch` or
  the Actions API. Do not declare done until `build`,
  `test-installer`, and `e2e` are all green.
- **One PR per concern.** If you find an unrelated bug while
  working, file it as a separate issue or PR.
...
- [ ] No new `TODO`, `FIXME`, or `XXX` in changed code. If the
      work is incomplete, finish it or leave it for a follow-up
      PR with a tracking issue.
...
- A new dependency is required → ask the user. Adding a
  dependency is a maintenance commitment, not a one-line change.
...
- [The AGENTS.md spec](https://agents.md/) — the canonical
  format and the list of supporting agents.

...
test.describe("Fix result content", () => {
  test("successful fix shows result with success indicator", async ({
    page,
  }) => {
    await ready(page);
    const row = page.locator("#findings tr[data-id='trivy.cve-2024-0002']");
...
test.describe("Fix modal for review finding", () => {
  test("review finding shows radio buttons and confirm is disabled", async ({
    page,
  }) => {
    await ready(page);
...
test.describe("Score breakdown penalty cap values", () => {
  test("vulnerabilities max penalty is 35", async ({ page }) => {
    ...
    const v = r.score_breakdown.axes.find(
  ...
  test("container_exposure max penalty is 30", async ({ page }) => {
    ...
    const ce = r.score_breakdown.axes.find(
  ...
  test("host_hardening max penalty is 25", async ({ page }) => {
    ...
    const hh = r.score_breakdown.axes.find(
  ...
  test("secrets max penalty is 10", async ({ page }) => {
    ...
    const s = r.score_breakdown.axes.find(
...
test.describe("Detail panel for different finding types", () => {
  test("trivy CVE shows all detail sections", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='trivy.cve-2024-0001']")
      .click({ force: true });
    await page.waitForTimeout(500);
  ...
  test("lynis finding shows correct metadata", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='lynis.AUTH-9286']")
      .click({ force: true });
    await page.waitForTimeout(500);
    const text = await page.locator("#detail").textContent();
  ...
  test("compose finding shows service", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='compose.dr004']")
      .click({ force: true });
    await page.waitForTimeout(500);
    const text = await page.locator("#detail").textContent();
  ...
  test("unavailable finding has no fix button", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='test.unfixable-001']")
      .click({ force: true });
    await page.waitForTimeout(500);
    const fixBtn = page.locator("#detail .fix-btn");
  ...
  test("fixed finding has no fix button", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='trivy.cve-2024-0003']")
      .click({ force: true });
    await page.waitForTimeout(500);
    const fixBtn = page.locator("#detail .fix-btn");
    expect(await fixBtn.count()).toBe(0);
...
test.describe("Table row states", () => {
  ...
  test("fixed row has check mark instead of badge", async ({ page }) => {
    await ready(page);
    const cell = page
      .locator("#findings tr[data-id='trivy.cve-2024-0003'] td")
      .nth(1);
...
test.describe("Selection behavior", () => {
  ...
  test("select-all checkbox selects all batch-selectable", async ({
    page,
  }) => {
    await ready(page);
...
test.describe("Keyboard navigation", () => {
  ...
  test("Escape blurs search input", async ({ page }) => {
    ...
    await page.waitForTimeout(100);
    expect(
      await query.evaluate((el) => el === document.activeElement)
    ).toBe(true);
    await page.keyboard.press("Escape");
    await page.waitForTimeout(100);
    expect(
...
test.describe("Score plate and metrics", () => {
  ...
  test("total metric shows 14", async ({ page }) => {
    await ready(page);
    expect(
      await page.locator("#metrics .metric").first().textContent()
    ).toContain("14");
  });
});
...
test.describe("Filter chip active state", () => {
  test("All chip is active by default", async ({ page }) => {
    ...
    const text = await page
      .locator("#severityFilters button.chip.active")
      .textContent();
...
test.describe("Detail panel copy button", () => {
  test("how_to_fix section has copy button", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='trivy.cve-2024-0002']")
      .click({ force: true });
    await page.waitForTimeout(500);
...
test.describe("Sort by source groups findings", () => {
  test("compose findings are grouped together", async ({ page }) => {
    ...
    const composeIdx = ids
      .map((id, i) => (id.startsWith("compose.") ? i : -1))
      .filter((i) => i >= 0);
...
test.describe("Responsive behavior", () => {
  test("score breakdown at 768px shows all axes", async ({ page }) => {
    ...
    await page.waitForTimeout(200);
    expect(
      await page.locator("#scoreBreakdown .score-axis").count()
    ).toBe(4);
...
test.describe("Export and help modals", () => {
  test("export modal has JSON, CSV, AI options and close button", async ({
    page,
  }) => {
    await ready(page);
    await page.keyboard.press("e");
...
test.describe("Topbar and findings panel structure", () => {
  test("topbar has hostveil title", async ({ page }) => {
    await ready(page);
    expect(await page.locator(".topbar h1").textContent()).toContain(
      "hostveil"
    );
  });
  ...
  test("findings panel has eyebrow", async ({ page }) => {
    await ready(page);
    expect(
      await page.locator(".findings-panel .eyebrow").textContent()
    ).toContain("Findings");
  });
...
test.describe("Tooltip text on buttons", () => {
  test("recalc button has title tooltip", async ({ page }) => {
    await ready(page);
    const title = await page.locator("#recalcBtn").getAttribute("title");
...
test.describe("Detail panel for different severity levels", () => {
  ...
  test("low finding shows low badge", async ({ page }) => {
    await ready(page);
    await page
      .locator("#findings tr[data-id='lynis.KRNL-5780']")
      .click({ force: true });
    await page.waitForTimeout(500);
    const badge = page.locator("#detail .badge");
    if ((await badge.count()) > 0) {
...
test.describe("Metrics medium and low counts", () => {
  test("medium metric shows 4", async ({ page }) => {
    await ready(page);
    expect(
      await page.locator("#metrics .metric").nth(3).textContent()
    ).toContain("4");
  });
  ...
  test("low metric shows 2", async ({ page }) => {
    await ready(page);
    expect(
      await page.locator("#metrics .metric").nth(4).textContent()
    ).toContain("2");
  });
});

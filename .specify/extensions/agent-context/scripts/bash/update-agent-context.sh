...
PLAN_PATH="${1:-}"
# If the user passed an absolute path that lives under the project root,
# strip the project-root prefix to keep the managed section portable
# (the auto-detection branch below does the same). An absolute path
# outside the project root is rejected.
if [[ -n "$PLAN_PATH" ]]; then
  if [[ "$PLAN_PATH" == "$PROJECT_ROOT"/* ]]; then

...
var checkerAsksForMore = map[string]string{
	...
	"ssh.clientaliveinterval":    "idle sessions may be intentionally long-lived",
	"ssh.maxsessions":            "multiplexed workflows may require several sessions",
	"ssh.tcpkeepalive":           "operators may prefer TCP liveness detection",
	"ssh.allowagentforwarding":   "administrative hops may depend on agent forwarding",
	"accounts.default-umask":     "shared group workflows may require broader defaults",
	...
	"ports.redis-protected-mode": "existing remote clients may depend on the current mode",

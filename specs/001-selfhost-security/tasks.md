...
- [X] T016 Write red tests for the logger in `internal/log/log_test.go` covering JSON output, correlation-id propagation, and level filtering
- [X] T017 Implement the SQLite store skeleton in `internal/store/store.go` opening the `state.db` with WAL mode, foreign keys on, busy timeout 5s, and the pragmas from `contracts/state-db.md` §"Global settings"
- [X] T018 Implement the migrations framework in `internal/store/migrations.go` with the `schema_migrations` table; add the first migration `0001_initial.sql` creating all tables from `contracts/state-db.md` (hosts, services, config_files, settings, container_images, vulnerabilities, container_image_vulnerabilities, scan_runs, findings, fixes, fix_records, suppressions, cve_cache_meta, tui_sessions, web_sessions, ai_providers, ai_requests)
- [X] T019 Write red tests for the store in `internal/store/store_test.go` and `internal/store/migrations_test.go` covering open/close, foreign-key enforcement, migration application, and idempotent re-application
...
- [X] T027 Write red tests for the common flags and exit codes in `internal/cli/flags_test.go` and `internal/cli/exitcode_test.go`

...
// The served default is what --layout, HOSTVEIL_LAYOUT or the remembered
// choice resolved to; an unknown or empty ID falls back to the shipped one,
// so a caller may pass whatever it has.
func layoutJS(startID string) string {
	if _, ok := LookupLayout(startID); !ok {
		startID = DefaultLayout().ID
	}

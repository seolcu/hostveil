...
func TestDeltaSummaryNamesWhatMoved(t *testing.T) {
	...
	before := model.NewFinding("cve.outdated-image", "t", model.SeverityMedium,
	...
	after := model.NewFinding("cve.outdated-image", "t", model.SeverityHigh,
		model.SourceCVE, model.RemediationReview, model.WithService("cloud/nextcloud"),
	...
	if !strings.Contains(out, "severity medium → high") {

...
var rules = []rule{
	...
	{id: "systemd.protect-kernel-modules", title: "A service can load or alter kernel modules", sev: model.SeverityMedium, desc: "ProtectKernelModules is off, so a compromised privileged service can modify the code running in the kernel.", directive: "ProtectKernelModules=yes", remediation: model.RemediationReview, flagged: func(u unit) bool { return known(u.ProtectKernelModules) && !on(u.ProtectKernelModules) }},
	...
	{id: "systemd.protect-kernel-logs", title: "A service can read or alter kernel logs", sev: model.SeverityLow, desc: "ProtectKernelLogs is off, so a compromised service may obtain sensitive kernel diagnostics or interfere with them.", directive: "ProtectKernelLogs=yes", remediation: model.RemediationReview, flagged: func(u unit) bool { return known(u.ProtectKernelLogs) && !on(u.ProtectKernelLogs) }},
	{id: "systemd.protect-clock", title: "A service can change the system clock", sev: model.SeverityLow, desc: "ProtectClock is off, so a privileged service can alter system or hardware clocks and undermine logs, certificates, and scheduled work.", directive: "ProtectClock=yes", remediation: model.RemediationReview, flagged: func(u unit) bool { return known(u.ProtectClock) && !on(u.ProtectClock) }},
	{id: "systemd.restrict-suid-sgid", title: "A service can create setuid or setgid files", sev: model.SeverityMedium, desc: "RestrictSUIDSGID is off, leaving a path for a compromised service to create files that grant privileges when executed later.", directive: "RestrictSUIDSGID=yes", remediation: model.RemediationReview, flagged: func(u unit) bool { return known(u.RestrictSUIDSGID) && !on(u.RestrictSUIDSGID) }},

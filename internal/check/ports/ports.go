...
func redisFindings(data []byte, path string) []model.Finding {
	...
	add := func(id, title, desc, how string, kind model.RemediationKind) {
		out = append(out, model.NewFinding(id, title, model.SeverityMedium, model.SourcePorts, kind,
	...
	if !regexpLine(text, `bind\s+(127\.0\.0\.1|localhost)(\s|$)`) {
		add("ports.redis-bind", "Redis is configured to listen beyond loopback", "Redis should normally be reached by local applications or a private proxy, not every network attached to the host.", "Bind Redis to 127.0.0.1 in "+path+" after confirming no remote client connects directly.", model.RemediationReview)
	}
	if !regexpLine(text, `protected-mode\s+yes(\s|$)`) {
		// Unlike its siblings, this carries no caveat: protected-mode yes is
		// Redis's own upstream default, and the fix only takes effect on the
		// next Redis restart (TakesEffectOn), so nothing live changes when it
		// is applied.
		add("ports.redis-protected-mode", "Redis protected mode is disabled", "Protected mode refuses unsafe remote access when authentication and binding are incomplete.", "Set protected-mode yes in "+path+".", model.RemediationAuto)
	}
	if !regexpLine(text, `rename-command\s+CONFIG\s+""(\s|$)`) {
		add("ports.redis-disable-config", "Redis exposes the CONFIG command", "A compromised Redis client can use CONFIG to rewrite runtime settings and persistence paths.", "Disable CONFIG with `rename-command CONFIG \"\"` in "+path+" after checking administration workflows.", model.RemediationReview)
	}
	return out
}

...
func defaultRules() []Rule {
	return []Rule{
		...
		{Path: "/boot/grub/grub.cfg", MaxMode: 0o600, Sev: model.SeverityHigh, ID: "fileperms.grub-config", Title: "The GRUB configuration is readable or writable beyond root", Desc: "The bootloader configuration contains kernel command lines and controls what starts before normal host protections are active."},
		{Path: "/boot/grub2/grub.cfg", MaxMode: 0o600, Sev: model.SeverityHigh, ID: "fileperms.grub2-config", Title: "The GRUB2 configuration is readable or writable beyond root", Desc: "The bootloader configuration controls the kernel and startup options before the operating system's access controls apply."},
		{Path: "/etc/at.allow", MaxMode: 0o600, Sev: model.SeverityMedium, ID: "fileperms.at-allow", Title: "The at allowlist is too permissive", Desc: "The at allowlist controls who may schedule future commands; only root should be able to change that policy."},
		{Path: "/etc/at.deny", MaxMode: 0o600, Sev: model.SeverityMedium, ID: "fileperms.at-deny", Title: "The at denylist is too permissive", Desc: "A user who can alter the at denylist can change who may schedule commands for later execution."},
		{Path: "/etc/cron.allow", MaxMode: 0o600, Sev: model.SeverityMedium, ID: "fileperms.cron-allow", Title: "The cron allowlist is too permissive", Desc: "The cron allowlist decides who may schedule recurring jobs and should be controlled only by root."},
		{Path: "/etc/cron.deny", MaxMode: 0o600, Sev: model.SeverityMedium, ID: "fileperms.cron-deny", Title: "The cron denylist is too permissive", Desc: "A user who can alter the cron denylist can change who is permitted to schedule recurring jobs."},
		{Path: "/etc/crontab", MaxMode: 0o600, Sev: model.SeverityHigh, ID: "fileperms.crontab", Title: "The system crontab is too permissive", Desc: "The system crontab can run commands as root; non-root access beyond what the scheduler needs creates an avoidable persistence surface."},
		{Path: "/etc/passwd-", MaxMode: 0o644, Sev: model.SeverityHigh, ID: "fileperms.passwd-backup", Title: "The passwd backup is writable by non-root users", Desc: "The backup account database can be restored later, so unauthorized changes can become active credentials after recovery work."},

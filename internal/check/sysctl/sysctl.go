...
func defaultRules() []Rule {
	return []Rule{
		...
		{ID: "sysctl.unprivileged-bpf", Title: "Unprivileged users can load BPF programs", Sev: model.SeverityMedium, Desc: "Unprivileged BPF exposes a large kernel attack surface to any local account and has repeatedly been involved in privilege escalation.", Keys: []string{"kernel.unprivileged_bpf_disabled"}, Want: "kernel.unprivileged_bpf_disabled = 1", Set: []string{"kernel.unprivileged_bpf_disabled=1"}, Flagged: is(0)},
		{ID: "sysctl.perf-events", Title: "Performance events are exposed to unprivileged users", Sev: model.SeverityMedium, Desc: "A permissive perf_event_paranoid value lets local users inspect other processes and kernel activity, leaking sensitive execution data.", Keys: []string{"kernel.perf_event_paranoid"}, Want: "kernel.perf_event_paranoid = 2", Set: []string{"kernel.perf_event_paranoid=2"}, Flagged: below(2)},

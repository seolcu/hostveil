...
func auditConfig(cfg sshdConfig, path string) []model.Finding {
	...
	rules := []hardeningRule{
		...
		{"ssh.tcpkeepalive", "TCPKeepAlive", "no", "yes", "SSH trusts unauthenticated TCP keepalive packets", "TCP keepalives are spoofable and do not prove the client is still present; encrypted client-alive messages provide the meaningful liveness check.", model.SeverityLow, model.RemediationAuto, func(v string) bool { return v != "no" }},

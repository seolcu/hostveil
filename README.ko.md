...
주고 원본을 백업한 뒤 고치며, 파일 변경은 롤백할 수 있습니다.

## 정말 효과가 있나요?

2026년 9월 11일, 의도적으로 취약하게 만든 Ubuntu VM에서 다시 측정했습니다.
Lynis, Docker의 CIS 벤치마크, Docker 브리지의 컨테이너에서 실행한 TCP 스캔,
커널의 수신 소켓을 비교했습니다. Nextcloud와 PostgreSQL, Jellyfin과 Redis,
Portainer와 Watchtower를 실행하며 root SSH 로그인을 허용하고 방화벽과 자동
업데이트를 끈 구성입니다.

| 측정 항목 | 수정 전 | Auto·Review 적용 및 재시작 후 |
| --- | --- | --- |
| **Docker 브리지에서 응답한 포트** | 6 | **1** |
| CIS Docker 벤치마크 (통과 / 경고) | 17 / 15 | **20 / 12** |
| Lynis 하드닝 지수 | 57 | **79** |
| Hostveil SSH 영역 | 10/100 | **100/100** |
| Hostveil 점수 | 28 | **58** |

소스 커밋 `290b4d1`을 `v3-dev`로 빌드하고 `scripts/measure/seed.sh`의 구성에서
얻은 결과입니다. [최종 실행 기록](docs/measurements/2026-09-11-cisc-w2026.md)에
원본 출력과 측정 방법이 있습니다. Review 조치와 서비스 재시작까지 포함하므로
`fix --all`만 실행한 결과와는 다릅니다.

체크포인트 79개를 롤백한 뒤, 변경된 경로 28개 중 25개를 정확히 복원했습니다.
**복원율은 89%입니다.** Review 수정 40건 중 16건은 되돌릴 것이 애초에 없습니다.
이 조치들은 `[not reversible]`로 표시됩니다. 계정 잠금 명령으로 바뀐
`/etc/shadow`, 패키지 업그레이드가 다시 쓴 `/etc/issue`와 `/etc/issue.net`에
변경이 남았습니다. 체크포인트 롤백 자체는 실패 보고 없이 완료됐지만 호스트
전체가 원래 상태로 돌아오지는 않았습니다.

컨테이너 점수는 0에 가깝게 남았고 CVE 점수는 개선되지 않았습니다.
Lynis 경고 3건 중 2건은 추가 UID 0 계정에 관한 것으로, Hostveil도 발견하고
수동 조치로 남깁니다. Review 적용 후 스캔에서는 SSH만 응답했지만 커널의
wildcard·라우팅 가능 수신 목록에는 SSH와 Redis가 모두 남았습니다.
수신 소켓과 실제로 도달 가능한 포트는 구분해야 합니다.

별도로 새 VM을 만들어 `control.sh`로 하드닝하고 Hostveil은 점검만 한 대조군에서는
점수가 **28 → 38**로 올랐습니다. 외부 응답 포트는 **7 → 2**로 줄었지만 수신
소켓은 **7**개로 같았습니다. 이 불일치의 원인은 해당 실행에서 규명하지 못했습니다.

전체 결과와 한계는 [측정 결과](https://hostveil.seolcu.com/ko/docs/measurements)
페이지에 있습니다. `scripts/measure/run.sh -c`는 폐기 가능한 VM이나 컨테이너에서만
실행해야 합니다. 이 검사는 되돌릴 수 없는 Review 조치의 잔여 변경을 허용하므로,
검사 통과가 모든 파일의 복원을 뜻하지는 않습니다.
...
Hostveil은 이 영역들에서 **발견 항목 175개**를 보고할 수 있고, 그중 **128개에
수정이 붙어 있습니다.** 75개는 무인으로 적용하고, 53개는 차이를 읽은 뒤에만

...
Jellyfin이나 Nextcloud, 게임 서버, 로컬 LLM처럼 셀프호스팅하는 서버 대부분은 설치할
때의 기본값을 그대로 씁니다. 기본값 하나만 잘못 두어도 서버를 통째로 잃을 수
...
수정이 바꾼 파일은 롤백으로 전부 바이트 단위까지 복원됐습니다. 체크포인트 48개에
걸쳐 8개 중 8개입니다. 검토 수정 18건 중 7건은 되돌릴 것이 애초에 없습니다.
파일 편집이 아니기 때문입니다. hostveil은 그것들을 히스토리에 `[not reversible]`로
표시해, 복원이 완전한 척하지 않습니다.

움직이지 않은 것도 중요합니다. 컨테이너 영역은 설계상 0에 가깝게 머뭅니다.
Portainer에 마운트된 Docker 소켓, 호스트 네트워킹, 환경 변수 속 비밀값처럼 놓친
게 아니라 수동으로 남긴 것들입니다. Lynis 지수는 57 → 60으로 움직였지만 Lynis
경고 4건 중 2건은 끝내 해소되지 않았습니다. 지수가 출력하지 않는 테스트까지
...
hostveil은 이 영역들에서 **발견 항목 174개**를 보고할 수 있고, 그중 **120개에
수정이 붙어 있습니다.** 71개는 무인으로 적용하고, 49개는 차이를 읽은 뒤에만
적용합니다. 나머지는 일부러 수동으로 둔 것이고,
[`internal/fix/register.go`](internal/fix/register.go)에 hostveil이 손대지 않는
이유가 하나하나 적혀 있습니다. 아무도 들여다보지 않아서 수동인 것이 아니라
판단해서 수동인 것이며, 그 차이에서 테스트가 빌드를 깨뜨립니다.
...
`fix --all`은 자동 수정만 적용합니다. `fix --all --review`를 쓰면 검토
...
어떤 항목이 자동 수정이 되려면 세 가지를 모두 만족해야 합니다.
...
셋 중 하나라도 어긋나면, 고를 대안이 있을 때는 검토가 되고 할 수 있는 일이
하나뿐이고 안전하게 만들 방법이 없을 때는 수동이 됩니다. 체커와 조치

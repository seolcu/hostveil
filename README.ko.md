...
self-hosting lab은 `scripts/self-hosting-lab.sh`로 관리합니다. 실행용 컨테이너는 `docker-compose.lab.yml`에 있고, 의도적으로 취약한 대상 스택은 `docker/lab/self-hosting-stack.yml`에 있습니다. 잘못 설정된 Vaultwarden, Jellyfin, Gitea, Nextcloud, PostgreSQL, nginx 서비스를 함께 띄워 실제 서버에 개발 빌드를 복사하지 않고도 Compose 파싱, 점수화, fix preview, TUI 동작을 검증할 수 있습니다.

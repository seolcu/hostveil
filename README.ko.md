...
Docker 워크플로는 작업 기준으로 정리되어 있습니다.

- `scripts/lab.sh dev`: 일반 Rust 개발 쉘
- `scripts/lab.sh host`: Fedora, Rocky, Ubuntu, Debian systemd lab에서 `setup`과 host scan 검증
- `scripts/lab.sh selfhost`: 의도적으로 취약한 self-hosting 스택과 TUI UX 검증

내부 Compose 파일은 의도적으로 분리해 둡니다.

- `compose.dev.yml`는 `dev`와 `host`를 담당합니다.
- `docker-compose.lab.yml`와 `docker/lab/self-hosting-stack.yml`는 `selfhost`를 담당합니다.

`compose.dev.yml`는 현재 다음 환경을 제공합니다.
...
배포판 lab 이미지는 공통 `docker/labs/systemd-lab.Dockerfile`을 공유하므로, 이후 다른 서비스를 추가하더라도 compose 수준 변경으로 유지할 수 있습니다.

`scripts/lab.sh selfhost ...` 뒤의 self-hosting lab은 실행용 컨테이너로 `docker-compose.lab.yml`를 쓰고, 의도적으로 취약한 대상 스택으로 `docker/lab/self-hosting-stack.yml`를 사용합니다. 잘못 설정된 Vaultwarden, Jellyfin, Gitea, Nextcloud, PostgreSQL, nginx 서비스를 함께 띄워 실제 서버에 개발 빌드를 복사하지 않고도 Compose 파싱, 점수화, fix preview, TUI 동작을 검증할 수 있습니다.

안전을 위해 helper script는 lab 서비스를 시작하기 전에 public 포트 바인딩을 `127.0.0.1`로 바꿉니다. 원본 Compose 파일에는 `0.0.0.0` 바인딩이 그대로 남아 있으므로 hostveil이 여전히 해당 finding을 보고합니다.

...
- **검토**: 유효한 대안이 여럿입니다. 하나를 고르세요.
- **수동**: 안전하게 자동화할 방법이 없어서, 대신 무엇을 해야 하는지
  설명합니다.
...
high 하나가 축의 절반, medium이 8분의 1, low가 16분의 1을 가져갑니다. 항상 남은
...
응답을 멈춘 다섯 포트는 PostgreSQL, Redis, Nextcloud, Jellyfin, Portainer입니다.
아직 응답하는 둘은 SSH와, 직접 설치된 Redis입니다. 뒤쪽은 hostveil이 찾아내고도
고치기를 거절한 항목인데, 설정 파일이 데이터스토어마다 달라서 발견 항목이 그
경로를 들고 있지 않기 때문입니다.

수정이 바꾼 파일은 롤백으로 전부 바이트 단위까지 복원됐습니다. 체크포인트 33개에
걸쳐 5개 중 5개입니다. 검토 수정 16건 중 7건은 되돌릴 것이 애초에 없고(이미지
갱신 6건과 unattended-upgrades 활성화), hostveil은 그것들을 히스토리에
`[not reversible]`로 표시합니다. 복원이 완전한 척하지 않습니다.

움직이지 않은 것도 함께 공개합니다. 컨테이너 영역은 끝까지 0/100인데, 남은 것이
설계상 수동이기 때문입니다. Portainer에 마운트된 Docker 소켓, 호스트 네트워킹,
환경 변수 속 비밀값이 그렇습니다. Lynis 경고 3건 중 2건은 hostveil이 찾아놓고
삭제를 거절한 두 번째 UID 0 계정입니다. `userdel`은 체크포인트로 되돌릴 수
없으니까요.

...
평범한 셀프호스팅 상자처럼 꾸민 실제 ARM64 서버에서 — Nextcloud+PostgreSQL,
Jellyfin+Redis, Portainer+Watchtower가 모든 포트를 `0.0.0.0`에 공개하고, root
SSH 로그인 허용, 방화벽 없음, 자동 업데이트 꺼짐:

| 무엇이 쟀나 | 적용 전 | `fix --all --review` 이후 |
| --- | --- | --- |
| **호스트 밖에서 응답하는 포트** | 7 | **2** |
| CIS Docker 벤치마크 (통과 / 경고) | 16 / 16 | **20 / 12** |
| Lynis 하드닝 지수 | 56 | **60** |
| hostveil SSH 도메인 | 18/100 | **100/100** |
| hostveil 점수 | 42 | **59** |

응답을 멈춘 다섯 포트는 PostgreSQL, Redis, Nextcloud, Jellyfin, Portainer입니다.
남은 둘은 SSH와, hostveil이 보고하고도 **고치기를 거절한** 네이티브 Redis입니다 —
설정 파일이 데이터스토어마다 다르고 발견 항목이 그 경로를 들고 있지 않기
때문입니다.

수정이 바꾼 파일은 롤백으로 전부 바이트 단위 복원됐습니다 — 체크포인트 33개에
걸쳐 5개 중 5개. 검토 수정 16건 중 7건은 되돌릴 것이 아예 없고(이미지 갱신 6건과
unattended-upgrades 활성화), hostveil은 그것들을 자신의 히스토리에 각각
`[not reversible]`로 표시합니다 — 복원이 완전한 척하지 않습니다.

**움직이지 않은 것도** 함께 실려 있습니다: 컨테이너 도메인은 끝까지 0/100인데
남은 것이 설계상 수동이기 때문이고(Portainer에 마운트된 Docker 소켓, 호스트
네트워킹, 환경 변수 속 비밀값), Lynis 경고 3건 중 2건은 hostveil이 찾아놓고
